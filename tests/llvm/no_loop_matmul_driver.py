...
from metalift.ir import (Expr, FnDecl, FnDeclRecursive, IntObject, TupleObject, Var, call, choose, ite, make_tuple)
...
def target_lang() -> List[FnDeclRecursive]:
    a = TupleObject("a", IntObject, IntObject)
    b = TupleObject("b", IntObject, IntObject)
    ...
    mat_mul = FnDecl(MAT_MUL, TupleObject[IntObject, IntObject], mat_mul_body.src, a.src, b.src, x.src)

    p = TupleObject("p", IntObject, IntObject)
    p0 = p[0]
    p1 = p[1]
...
def ps_grammar(ret_val: Var, writes: List[Var], reads: List[Var]) -> Expr:
    ...
    b = make_tuple(b0, b1)
    x = make_tuple(x0, x1)
    p = call(MAT_MUL, TupleObject[IntObject, IntObject], a, b, x)
    wrong_p = call(MAT_MUL, TupleObject[IntObject, IntObject], b, a, x)
    wrong_p2 = call(MAT_MUL, TupleObject[IntObject, IntObject], a, x,  b)
